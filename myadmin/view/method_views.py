...
from myadmin.models import Method
...
def get_list(request):
    ...
    data_list = Method.objects.all().filter(Q(name__icontains=keyword)).order_by(order_by)
...
def add(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            data = Method.objects.create(module_id=True)
            data.name = request.POST.get('name')
            data.display_name = request.POST.get('display_name')
            data.module_id = request.POST.get('module')
            data.status = True
            data.save()
...
def edit(request, id):
    ...
    if request.method == 'POST':
        ...
        obj = Method.objects.get(pk=id)
        form = MethodForm(instance=obj)
    cont = {
        'page_name': page_name,
        'page_url': page_url,
        'form': form,
