...
def get_list(request):
    checkRolePermission(request, page_url + "-list")
    sortable_columns = ['id', 'rating_for', 'rating_from', 'rating', 'comment']
    ...
    print('==================================39')
    data_list = Rating.objects.all().filter(Q(rating_for__studio_name__icontains=keyword) | Q(rating_from__first_name__icontains=keyword) | Q(rating__icontains=keyword)).order_by(order_by)
    print(data_list,'==============================41')
    ...
    for record in records:
        ...
        u = {
            'sno': start + i,
            'rating_for': record.rating_for.studio_name,
            'rating_from': record.rating_from.first_name,
